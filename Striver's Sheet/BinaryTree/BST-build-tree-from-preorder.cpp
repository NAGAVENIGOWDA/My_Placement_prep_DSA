...
class Solution {
  public:
    ...
    Node* Bst(int pre[], int size) {
        ...
        return build(pre, idx, size, INT_MAX);
    }
    
    Node* build(int pre[], int& idx, int size, int bound) {
        if (idx == size || pre[idx] > bound) 
            return NULL;
        
        Node* node = newNode(pre[idx++]);
        node->left = build(pre, idx, size, node->data); // Update the bound for left subtree
        node->right = build(pre, idx, size, bound); // Propagate same bound for right subtree
